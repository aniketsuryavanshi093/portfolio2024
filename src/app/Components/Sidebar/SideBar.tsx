      ...
      {sidebarobject?.map((elem: sidebaroptions) => (
        <Tooltip key={elem.icon} content={elem.toltipcontent} placement="right">
          <Link href={elem.redirecthref}>
            <div
              onClick={() => setSelectedIcon(elem.toltipcontent)}
              className={`${
                SelectedIcon === elem.toltipcontent && "selectedicon"
              } cursor-pointer sidebaricons h-12 w-12  flex flex-col justify-center align-center`}
            >
